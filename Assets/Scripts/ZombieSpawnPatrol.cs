...
public class ZombieSpawnPatrol : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.CompareTag("Player") && canSpawn == true && SaveScript.zombiesInGameAmt >= 140 - zombieSpawnAmt)
        {
            GameObject[] zombiesToDestroy = GameObject.FindGameObjectsWithTag
            ("zombie");
    ...
    void SpawnZombies()
    {
        for (int i = 0; i < zombieSpawnAmt; i++)
        {
            if (houseSpawn == false)
            {
                ...
                Instantiate(zombies[Random.Range(0, zombies.Length)], new
                Vector3(spawnPoints[spawnRandom].position.x - Random.Range(0,10), 
                spawnPoints[spawnRandom].position.y, spawnPoints
                [spawnRandom].position.z - Random.Range(0, 5)), spawnPoints
                [spawnRandom].rotation);
            }
            else
            {
